...
def crear():
    ...
    db.session.close()
    return redirect(url_for("home"))  # retorno a home
...
@app.route("/comentario-tarea/<id>", methods=["POST"])
def comentario(id):
    tarea = db.session.query(Tarea).filter_by(id=id).first()
    tarea.comentario= request.form['comentario_tarea']
    db.session.commit()  # Ejecutar la operación pendiente de la base de datos
    db.session.close()
    return redirect(url_for("home"))  # retorno a home

@app.route("/fecha-tarea/<id>", methods=["POST"])
def fecha(id):
    tarea = db.session.query(Tarea).filter_by(id=id).first()
    tarea.fecha= request.form['fecha_tarea']
    db.session.commit()  # Ejecutar la operación pendiente de la base de datos
    db.session.close()
    return redirect(url_for("home"))  # retorno a home
